...
TEST_CASE( "Bi-directional communiction." )
{
    ...
    client->set_message_handler( [ ]( const shared_ptr< Adaptor > client )
    {
        fprintf( stderr, "Client message handler called.\n" );
        error_code status;
        auto data = client->consume( status );
        REQUIRE( status == error_code( ) );
        REQUIRE( data == make_bytes( "Hello, Client!" ) );
    ...
    {
        fprintf( stderr, "Server open handler called.\n" );
    ...
    {
        fprintf( stderr, "server message handler called.\n" );
        error_code status;
